...
const manifestPath=path.join(root,'data','canonical-route-manifest-v43.json');
if(!fs.existsSync(manifestPath)){console.error('FAIL · missing canonical-route-manifest-v43.json'); process.exit(1)}
...
if(fail.length){console.error('FAIL · public trust v43 checks failed'); for(const f of fail.slice(0,80)) console.error(' - '+f); if(fail.length>80) console.error(` - ... ${fail.length-80} more`); process.exit(1);}
console.log(`PASS · public trust v43 metadata, exact route count, and no Loading fallbacks across ${manifest.pages.length} routes`);
