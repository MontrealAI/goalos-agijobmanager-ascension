...
// Compatibility manifest: create v47 from v45/v43 if needed.
if(!fs.existsSync('data/canonical-route-manifest-v47.json')){
  ...
  if(src){ const m=JSON.parse(fs.readFileSync(src,'utf8')); m.release=release; m.version='v47'; m.releaseAliases=aliases; if(Array.isArray(m.pages)) m.routeCount=m.pages.length; fs.writeFileSync('data/canonical-route-manifest-v47.json', JSON.stringify(m,null,2)); }
}
// Make old cleanup aliases safe.
ensureCopy('tools/root-cleanup-v45.mjs','tools/root-cleanup-v47.mjs');
if(!fs.existsSync('tools/root-cleanup-v47.mjs')) write('tools/root-cleanup-v47.mjs', `import fs from 'node:fs';
import path from 'node:path';
...
console.log('PASS · root cleanup v47 moved/merged '+moved+' historical files');
`);
ensureCopy('tools/root-cleanup-v47.mjs','tools/root-cleanup-v45.mjs');
ensureCopy('tools/root-cleanup-v47.mjs','tools/root-cleanup-v44.mjs');
ensureCopy('tools/root-cleanup-v47.mjs','tools/root-cleanup-v43.mjs');
...
fs.writeFileSync('data/site-rehydration-v47.json', JSON.stringify({status:'PASS',restored:0,release:'v47'},null,2));
console.log('PASS · fallback site rehydration v47 restored 0 missing files');
`);
// Public trust checker aliases.
ensureCopy('tools/public-trust-checker-v47.mjs','tools/public-trust-checker-v45.mjs');
ensureCopy('tools/public-trust-checker-v47.mjs','tools/public-trust-checker-v43.mjs');
ensureCopy('tools/apply-public-trust-metadata-v45.mjs','tools/apply-public-trust-metadata-v47.mjs');
if(!fs.existsSync('tools/apply-public-trust-metadata-v47.mjs')) write('tools/apply-public-trust-metadata-v47.mjs', `import fs from 'node:fs'; import path from 'node:path';
...
console.log('PASS · public trust metadata v47 applied');
`);
// Copy v47 files backward for v45/v44 compatibility if current workflow/kernels ask for old names.
ensureCopy('tools/repository-public-trust-compatibility-failsafe-v47-kernel.mjs','tools/repository-public-trust-failsafe-v45-kernel.mjs');
ensureCopy('tests/repository-public-trust-compatibility-failsafe-v47.test.mjs','tests/repository-public-trust-failsafe-v45.test.mjs');
console.log('PASS · failsafe bootstrap v47 completed');
