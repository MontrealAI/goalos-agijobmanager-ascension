...
if(!fs.existsSync('site/asi-proof-horizon-console.html')) fail('missing ASI Proof Horizon Console page');
const data=JSON.parse(fs.readFileSync('data/asi-proof-horizon-console-demo.json','utf8'));
if(!data.posture?.noWallet || !data.posture?.noUserDataWanted || !data.posture?.zeroNetwork) fail('ASI Proof Horizon data contract public-safe posture incomplete');
if(!data.claimBoundary || !/No achieved AGI/i.test(data.claimBoundary) || !/achieved ASI/i.test(data.claimBoundary)) fail('ASI Proof Horizon claim boundary incomplete');
ok('public trust checker v53 passed');
