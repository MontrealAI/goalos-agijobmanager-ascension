...
if(!(/Publisher v4[456]/.test(wf) || wf.includes('Public Trust Ultimate Failsafe Publisher v45') || wf.includes('Compatibility Failsafe Publisher v46'))) fail('workflow name must advertise v44-compatible, v45, or v46 release');
pass('workflow advertises v44-compatible/v45/v46 release');
...
for (const f of ['tools/root-cleanup-v46.mjs','tools/apply-public-trust-metadata-v46.mjs','tools/public-trust-checker-v46.mjs','tools/repository-public-trust-compatibility-failsafe-v46-kernel.mjs']) if(!fs.existsSync(f)) fail(`source missing v45 public-trust tool: ${f}`);
pass('v46 public-trust source tools exist');
const pkg=JSON.parse(read('package.json'));
if(!/4\.[56]\.0|v4[56]/.test(String(pkg.version||''))) fail('package version must include v45-compatible or v46');
