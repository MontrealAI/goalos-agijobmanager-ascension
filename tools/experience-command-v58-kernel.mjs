...
ok(manifest.version==='v58','canonical manifest is v58');
...
if(fs.existsSync('dist/build-manifest.json')){const bm=JSON.parse(fs.readFileSync('dist/build-manifest.json','utf8')); ok(String(bm.release||'').includes('v58'),'dist build manifest advertises v58'); ok(Number(bm.routeCount)>=manifest.routeCount,'dist build manifest does not lose routes');}
