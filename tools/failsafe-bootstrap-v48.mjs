...
// Compatibility manifest: create v48 from v45/v43 if needed.
if(!fs.existsSync('data/canonical-route-manifest-v48.json')){
  ...
  if(src){ const m=JSON.parse(fs.readFileSync(src,'utf8')); m.release=release; m.version='v48'; m.releaseAliases=aliases; if(Array.isArray(m.pages)) m.routeCount=m.pages.length; fs.writeFileSync('data/canonical-route-manifest-v48.json', JSON.stringify(m,null,2)); }
}
// Make old cleanup aliases safe.
ensureCopy('tools/root-cleanup-v45.mjs','tools/root-cleanup-v48.mjs');
if(!fs.existsSync('tools/root-cleanup-v48.mjs')) write('tools/root-cleanup-v48.mjs', `import fs from 'node:fs';
import path from 'node:path';
...
console.log('PASS · root cleanup v48 moved/merged '+moved+' historical files');
`);
ensureCopy('tools/root-cleanup-v48.mjs','tools/root-cleanup-v45.mjs');
ensureCopy('tools/root-cleanup-v48.mjs','tools/root-cleanup-v44.mjs');
ensureCopy('tools/root-cleanup-v48.mjs','tools/root-cleanup-v43.mjs');
...
fs.writeFileSync('data/site-rehydration-v48.json', JSON.stringify({status:'PASS',restored:0,release:'v48'},null,2));
console.log('PASS · fallback site rehydration v48 restored 0 missing files');
`);
// Public trust checker aliases.
ensureCopy('tools/public-trust-checker-v48.mjs','tools/public-trust-checker-v45.mjs');
ensureCopy('tools/public-trust-checker-v48.mjs','tools/public-trust-checker-v43.mjs');
ensureCopy('tools/apply-public-trust-metadata-v45.mjs','tools/apply-public-trust-metadata-v48.mjs');
if(!fs.existsSync('tools/apply-public-trust-metadata-v48.mjs')) write('tools/apply-public-trust-metadata-v48.mjs', `import fs from 'node:fs'; import path from 'node:path';
...
console.log('PASS · public trust metadata v48 applied');
`);
// Copy v48 files backward for v45/v44 compatibility if current workflow/kernels ask for old names.
ensureCopy('tools/repository-public-trust-compatibility-failsafe-v48-kernel.mjs','tools/repository-public-trust-failsafe-v45-kernel.mjs');
ensureCopy('tests/repository-public-trust-compatibility-failsafe-v48.test.mjs','tests/repository-public-trust-failsafe-v45.test.mjs');
console.log('PASS · failsafe bootstrap v48 completed');
